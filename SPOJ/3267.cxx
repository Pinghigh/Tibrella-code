...
#pragma GCC target("sse,sse2,sse3,ssse3,sse4.1,sse4.2,avx,avx2,popcnt,lzcnt,bmi,bmi2")
...
namespace pdqsort_detail {
    ...
    template <class T>
    inline unsigned long long log2(T n) {
    ...
    inline void pdqsort_loop(Iter begin, Iter end, Compare comp, unsigned long long bad_allowed, bool leftmost = true) {
...
unsigned long long lis[N];
unsigned long long n, q;

unsigned long long block, idx, hav;
...
struct ele {
    ...
    void read(unsigned long long idx) {
        cin >> l >> r;
...
signed main() {
    ...
    pdqsort(que + 1, que + q + 1, [](ele& a, ele& b) {
    ...
    l = r = 0;
    for (unsigned long long i = 1; i <= q; ++i) {
        unsigned long long ql = que[i].l, qr = que[i].r;
        while (l < ql)
            del(l++);
        while (l > ql)
            ins(--l);
