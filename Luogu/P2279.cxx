...
inline int lg(long long __n) {
    return sizeof(long long) * __CHAR_BIT__ - 1 - __builtin_clz(__n);
}
...
inline void pdqsort(Iter begin, Iter end, Compare comp) {
    ...
    pdqsort_detail::pdqsort_loop<Iter, Compare,
        ...
        begin, end, comp, lg(end - begin));
#else
    pdqsort_detail::pdqsort_loop<Iter, Compare, false>(begin, end, comp, lg(end - begin));
#endif
}
...
inline void pdqsort_branchless(Iter begin, Iter end, Compare comp) {
    if (begin == end) return;
    pdqsort_detail::pdqsort_loop<Iter, Compare, true>(begin, end, comp, lg(end - begin));
}
...
int main() {
    ...
    pdqsort_branchless(nod + 1, nod + n + 1, [](const i32& a, const i32& b) { return dep[a] > dep[b]; });

    for (int i = 1; i <= n; ++i) {
        i32 x = nod[i], y = nod[i];
        for (int d = 1; d <= k; ++d) {
            y = fa[y];
            dis[x] = std::min(dis[x], dis[y] + d);
        }
        if (dis[x] > k) {  // 距离最近的关键点距离大于 k
            dis[y] = 0;
            for (int d = 1; d <= k; ++d) {
                y = fa[y];
                dis[y] = std::min(d, dis[y]);
            }
            ++ans;
        }
    }
